...
Typing replaces the active selection, and Backspace/Delete remove it at complete text-element boundaries. `Ctrl+W` toggles wrap; `Ctrl+V` starts/stops selection; `Ctrl+G` or `Ctrl+K Ctrl+G` opens the 1-based record prompt. Within that explicit prompt scope, `Ctrl+K Ctrl+C` or `Escape` cancels and `Enter` accepts. Arrows navigate; `PageUp`/`PageDown` jump records; `Home`/`End` jump within the current record; typing, `Tab`, `Enter`, `Backspace` and `Delete` edit. `Escape` exits from the document context. Terminal control-key reporting may differ by terminal. Resize recomputes the document, status, prompt and interaction regions and invalidates any live prefix. Minimum size is 30 columns by 6 rows.
