...
public sealed partial class CursesInteractionRegion {
	...
	/// <summary>Binds one ordered gesture sequence to an application command in this region.</summary>
	...
	public void BindGestureSequence(
		...
		if ( this.gestureBindings.ContainsKey( registration.Gestures[0] ) ) {
			throw new InvalidOperationException(
				"The interaction region already has a single-key binding for this sequence prefix."
			);
		}
		...
		if ( CursesInteractionRouter.MaximumRegionCommandSequenceBindings
			...
			throw new InvalidOperationException(
				$"An interaction region cannot own more than {CursesInteractionRouter.MaximumRegionCommandSequenceBindings} command sequence bindings."
			);
		}
	...
	/// <summary>Removes one exact command-sequence binding from this region.</summary>
	...
	private static void EnsureNoSequenceConflict(
		...
		foreach ( CursesCommandSequenceRegistration registration in registrations ) {
			if ( CursesCommandSequenceRegistration.SequenceEquals(
				...
				throw new InvalidOperationException(
					"The interaction region already has a duplicate or ambiguous command sequence binding."
				);
			}
		}
	}
