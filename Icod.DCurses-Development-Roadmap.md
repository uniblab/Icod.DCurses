...
**Current development package:** `1.3.0-alpha.1`  
...
| `1.3.0` | Layout and resize primitives | Active development; T1310 gate |
...
Current 1.3 candidate contract:
...
## 1.3 accepted architecture to date
...
```text
T1301-T1309  implementation + acceptance                         complete
T1310        public API / package / docs / licensing regret gate in progress
T1311        RC + stable-source closure                          pending
```

T1310 is not complete until the exact head containing its package-only smoke, API baseline/fingerprint, README, roadmap, package metadata, and licensing-audit changes passes the full seven-job PR matrix.

After T1310, T1311 may promote the unchanged implementation/API to `1.3.0-rc.1`, qualify that exact head, then promote the unchanged implementation/API to stable-source `1.3.0` and qualify again.
