...
public sealed class CursesRefreshEngineTests {
	[Fact]
	public async Task SmallChangeDoesNotRepaintUnchangedScreen() {
		...
		Assert.Contains( "<cup:0,2>", rendered );
		Assert.Contains( "X", rendered );
		Assert.DoesNotContain( "<cup:1,0>", rendered );
	}
	...
	private sealed class RecordingOutput
		...
		internal string Text => Encoding.UTF8.GetString( bytes.ToArray() );
