...
**Status:** RC integration validation passed for PR head `591a26532ec2e0fe4d637732e3f826cebd4385b0` with `Version`/`PackageVersion` `2.0.0-rc.1`, `AssemblyVersion` `2.0.0.0` and direct `Icod.Terminal 1.18.0` only.
...
The PR workflow checked out GitHub's synthetic merge commit for this run, and package repository metadata named that synthetic commit. The stable-source workflow will check out the PR head explicitly so its package provenance points to the reviewed source commit.
