...
1. **RC identity.** Update `Version`, `PackageVersion`, package release notes, the 2.1 fingerprint release/status metadata and the exact-identity test to `2.1.0-rc.1`. Keep `AssemblyVersion` `2.0.0.0`. Run the exact PR-head 14-job Staging/Release runtime and package matrix. Check package-only .NET 8/9/10 consumers and source commit provenance in the package.
...
3. **Stable-source identity.** Update version/package notes/fingerprint release metadata/identity test to `2.1.0` without changing the accepted public surface or production behavior. Synchronize README, changelog and sample links to describe the stable-source candidate accurately while avoiding a publication claim. Run another exact-head 14-job matrix and fresh package-only consumer validation.
4. **Provenance and handoff.** Record the stable executable head, CI run, package and symbol package hashes, dependency/license/README/XML checks, no-merge state, and manual sample limits. If an evidence-only commit follows, require its own exact-head CI before a readiness claim; put the final head and hashes in the PR description without self-referential source commits.

## Acceptance

RC and stable-source identities each pass the full matrix at their own source head. The stable candidate package carries version `2.1.0`, only direct `Icod.Terminal 1.18.0`, .NET 8/9/10 assets, license, README, XML docs and portable symbols, and its provenance points to the checked-out PR head. PR #33 stays open and unmerged for maintainer review.
