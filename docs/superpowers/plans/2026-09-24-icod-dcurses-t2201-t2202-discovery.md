...
- [x] **Step 2: Write the additive discovery design.** Specify `CursesCommandBinding` and `GetEffectiveGestureBindings`, read-only detached snapshots, insertion and owner ordering, modal exclusion, limits, and deferred prompt/timing scope in `docs/2.2-Interaction-API-Design.md`.
...
- [ ] **Step 4: Commit the foundation documents.** Commit both updated roadmaps, the evidence and API design, and this plan on PR #34; verify the exact changed-file list on GitHub. Expected: no production code or package identity change.
...
- [ ] **Step 1: Write an executable first RED test.** In `CursesInteractionDiscoveryTests`, include GPL test header and `using Xunit;`:
...
- [ ] **Step 2: Observe the missing-API RED on PR CI.** Commit only the test and push using the connected GitHub integration. Wait for one exact-head runtime job to fail with `CS0246`/`CS1061` for the missing snapshot API. Cancel remaining redundant jobs after the representative RED result if necessary; never describe the expected error as an observed result until logs confirm it.
- [ ] **Step 3: Implement only discovery.** Add an immutable `CursesCommandBinding` that rejects invalid/default gestures and null commands. Add internal `IEnumerable<KeyValuePair<CursesKeyGesture,CursesCommand>> GestureBindings` properties to region and scope. In router's new partial file, call `ThrowIfDisposed()` and `RepairFocusIfNeeded()`, traverse eligible owner dictionaries in the design's precedence order and keep the first occurrence of each gesture in a `HashSet<CursesKeyGesture>`. Copy the results to an array and return `Array.AsReadOnly(array)`; do not mutate `Route`.
- [ ] **Step 4: Advance identity for implemented work and observe GREEN.** Set `Version` and `PackageVersion` to `2.2.0-alpha.1` together, keep assembly `2.0.0.0`, write package release notes for binding discovery only. Commit and push; require exact-head Staging PR runtime and package jobs to pass on all three TFMs. Verify new snapshot remains read-only by casting to `IList<CursesCommandBinding>` and asserting mutation throws `NotSupportedException`.
